...
void clkinit(void)
{
    ...
    *clkctrl = AM335X_TIMER1MS_CLKCTRL_EN;
    ...
    sleepq = newqueue(); /* Allocate a queue to hold the delta list of sleeping processes */
    ...
    /* The following values are calculated for a timer that generates 1ms tick rate	*/

    csrptr->tpir = 232000;
	csrptr->tnir = -768000;
	csrptr->tldr = 0xFFFFFFFF - 32;

    /* Set the timer to auto reload */

	csrptr->tclr = AM335X_TIMER1MS_TCLR_AR;

    /* Enable overflow interrupt which will generate an interrupt every 1 ms */
